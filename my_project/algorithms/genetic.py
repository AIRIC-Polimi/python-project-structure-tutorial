...
class GeneticAlgorithm:
    ...
    def __init__(self, target_string, population_size, mutation_rate, random_seed=None, log_level=None, log_file=None):
        ...
        self.tokens = np.array([ord(token) for token in [" "] + list(string.ascii_letters)])
    ...
    def run(self, iterations):
        ...
        for epoch in tqdm(range(iterations)):
            ...
            self.logger.debug(
                "[%d Closest Candidate: '%s', Fitness: %.8f]"
                % (epoch, "".join([chr(t) for t in fittest_individual]), highest_fitness)
            )
