...
from my_project.data.preprocess import train_test_split, k_fold_cross_validation_sets
...
def parse_config(print_config=False):
    ...
    parser.add_argument("--test-perc", type=float, default=0.2, help="The percentage of data to reserve for the test split.")
    parser.add_argument("--poly-degree", type=int, default=15, help="The maximum degree of polynomial to use.")
    parser.add_argument("--learning-rate", type=float, default=1e-3, help="The learning rate to use for gradient descent.")
    parser.add_argument("--iterations", type=int, default=10000, help="The number of iterations to run the gradient descent.")
    parser.add_argument("-k", "--k-fold", type=int, default=10, help="The k to use for k fold crossvalidation of hyperparameters.")
    parser.add_argument("--ridge-regression", default=False, action="store_true", help="Whether to use ridge regression regularization.")
    parser.add_argument("--min-regularization-factor", type=float, default=0, help="The minimum regularization factor to try when using ridge regression.")
    parser.add_argument("--max-regularization-factor", type=float, default=0.1, help="The maximum regularization factor to try when using ridge regression.")
    parser.add_argument("--regularization-factor-step", type=float, default=0.01, help="The step to use when trying diffent regularization factor values.")
    parser.add_argument("--figures-folder", type=str, default=path.abspath(path.join(path.dirname(__file__), '../figures')), help="The path of the folder where to store figures")
...
def run():
    ...
    if config.ridge_regression:
        ...
        for reg_factor in np.arange(config.min_regularization_factor, config.max_regularization_factor, config.regularization_factor_step):
            cross_validation_sets = k_fold_cross_validation_sets(
                X_train, y_train, k=config.k_fold)
            mse = 0
